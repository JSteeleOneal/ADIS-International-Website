...
Premium marketing site built with Next.js (App Router), React, TypeScript, Tailwind CSS, and Framer Motion, plus a working serverless contact form that emails inquiries to `ISZ.ADIS@protonmail.com` via Resend.

## Site structure

- `/` — home page (hero, about, solutions, contact form)
- `/products` — full ABZ Innovation product catalogue: L-Series spraying drones (L10 Pro, L30, L50), M-Series survey platform, C10 cleaning drone, spraying-technology comparison, field case studies, and differentiators

## Language switching (EN / PT)

The whole site is bilingual — English and European Portuguese. A sliding EN/PT toggle sits in the navbar (desktop and mobile). The choice is stored in `localStorage` and restored on the next visit; `<html lang>` is kept in sync for SEO and screen readers.

- UI copy lives in `lib/translations.ts`
- Product catalogue copy lives in `data/products.ts` (both languages, one typed `Catalogue` per language)
- The provider/hook is `lib/i18n.tsx` — to add a language, extend the `Lang` type and add a dictionary to both files
